...
function startAjax(biNr) {
    ...
    $.ajax({
        ...
        success: function(daten) {
            ...
            console.log('Upload response typeof:', typeof daten, daten);
                // If daten is a string, try to parse as JSON
                if (typeof daten === 'string') {
                    try {
                        daten = JSON.parse(daten);
                        console.log('Parsed JSON daten:', daten);
                    } catch(e) {
                        console.log('JSON parse error:', e, daten);
                    }
                }
           // bilder[biNr] = daten;
