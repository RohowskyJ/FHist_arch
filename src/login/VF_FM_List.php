...
function modifyRow(array &$row, $tabelle)
{
    ...
    switch ($s_tab) {
        case "fh_eigen":
            $ei_id = $row['ei_id'];
            $row['ei_id'] = "<a href='VF_FM_List.php?ei_id=$ei_id' >" . $ei_id . "</a>";
            break;
        case "mu_fahrz":
            $fm_id = $row['fm_id'];
            $row['fm_id'] = "<a href='VF_FM_FZ_Edit.php?fm_id=$fm_id' >" . $fm_id . "</a>";
            if ($row['fm_foto_1'] != "") {
        ...
        case "mu_gerae":
            $mg_id = $row['mg_id'];
            $row['mg_id'] = "<a href='VF_FM_GE_Edit.php?mg_id=$mg_id' >" . $mg_id . "</a>";
            if ($row['mg_foto_1'] != "") {
