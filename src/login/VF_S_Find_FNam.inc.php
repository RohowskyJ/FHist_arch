 ...
 */
$_SESSION[$module]['Inc_Arr'][] = "VF_S_Find_NamF.inc.php";
...
echo "<pre class=debug style='background-color:lightblue;font-weight:bold;'>SU FNam  $sql_zt </pre>";
echo "</div>";
...
foreach ($zt_arr as $file=>$v) {
    ...
    $sql = "SELECT ih_id,ih_fwehr FROM $file WHERE ih_fwehr LIKE '%$s_suchffname%' "; 
    $return = SQL_QUERY($db,$sql);
    while ($row = mysqli_fetch_object($return)) {
        if (empty($zeitg_liste)) {
            $zeitg_liste = "$file|$row->ih_id";
        } else {
            $zeitg_liste .= ",$file|$row->ih_id";
        }
    }
