...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($debug_log) {
        ...
        file_put_contents('debug.log', "L 035 $eintragen" . PHP_EOL, FILE_APPEND);
    }
    ...
    if (isset($_FILES['file']) && $_FILES['file']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array(strtolower($extension), $allowed_extensions)) {
            ...
            if ($debug_log) {file_put_contents('debug.log', "L 089 Uploadfile  $uploadFile \n " . PHP_EOL, FILE_APPEND);}

            // Datei hochladen
            if (move_uploaded_file($tmpName, $uploadFile)) {
               
                if ($debug_log) {file_put_contents('debug.log', "L 094 hochgeladen \n" . PHP_EOL, FILE_APPEND);}
                ...
                if ($debug_log) {file_put_contents('debug.log', "L 0105 Outputfile  $outputFile \n" . PHP_EOL, FILE_APPEND);}

                if (in_array(strtolower($extension), GrafFiles)) { // bearbeiten
                    
                    if ($debug_log) {file_put_contents('debug.log', "L 0109 fotobearb? \n" . PHP_EOL, FILE_APPEND);}
                    

                    $InputFile = '../../' . $targ_pfad . $name;
   
                    if ($debug_log) {file_put_contents('debug.log', "L 0113 InputFile $InputFile \n outputFile $outputFile \n" . PHP_EOL, FILE_APPEND);}
                    ...
                    if (!empty($rotateLeft)) {
                        foreach ($rotateLeft as $fileName) {
                            ...
                            if ($debug_log) {file_put_contents('debug.log', "L 0103 Datei $fileName soll links gedreht werden. \n " . PHP_EOL, FILE_APPEND);}
                        }
                        if (in_array($name, $rotateLeft)) {
                            if ($debug_log) {file_put_contents('debug.log', "L 0117 Datei $name soll rechts gedreht werden. \n " . PHP_EOL, FILE_APPEND);}
                            $rot_left = True;
                        }
                    }
                    ...
                    if (!empty($rotateRight)) {
                        foreach ($rotateRight as $fileName) {
                            ...
                            if ($debug_log) {file_put_contents('debug.log', "L 0116 Datei $fileName soll rechts gedreht werden. \n " . PHP_EOL, FILE_APPEND);}
                        }
                        if (in_array($name, $rotateRight)) {
                            if ($debug_log) {file_put_contents('debug.log', "L 0119 Datei $name soll rechts gedreht werden. \n " . PHP_EOL, FILE_APPEND);}
                            $rot_right = True;
                        }
                    }
                    ...
                    if ($urh_einfueg == "J") {
                        ...
                        if ($debug_log) {file_put_contents('debug.log', "L 0129 Urheber ins Bild Ja \n" . PHP_EOL, FILE_APPEND);}
                        
                        $ttf_file = "../Fonts/arialbd.ttf";
                        
                        $CR_text = '© '.$urh_name;
                        
                        if ($debug_log) {file_put_contents('debug.log', "L 0131 CR Text $CR_text \n" . PHP_EOL, FILE_APPEND);}
                        ...
                        if ($res_file == "") {
                            ...
                            if ($debug_log) {file_put_contents('debug.log', "L 0137 Fehler beim Resizing des Bildes. \n" . PHP_EOL, FILE_APPEND);}
                            
                        } else {
                            $response['valid_files'][] = $res_file; // Erfolgreich hochgeladene Datei speichern
                        }
                    } else {
                        if ($debug_log) {file_put_contents('debug.log', "L 0143 vor resize img ohne cd text \n" . PHP_EOL, FILE_APPEND);}
                        # resizeImage($inputFile, 800, 600, $outputFile); // Maximal 800x600
                        
                        $res_file = !resizeImage($InputFile, 800, 800, $outputFile);
                        if ($res_file == "") {
                            
                            if ($debug_log) {file_put_contents('debug.log', "L 0149 Fehler beim Resizing des Bildes. \n" . PHP_EOL, FILE_APPEND);}
                        } else {
                            $response['valid_files'][] = $res_file; // Erfolgreich hochgeladene Datei speichern
                        }
                    }
                ...
                if ($debug_log) {file_put_contents('debug.log', "L 0156 Urheber ins Bild ?? \n" . PHP_EOL, FILE_APPEND);}

            }
            
            if ($debug_log) {file_put_contents('debug.log', "L 0160 ende hochladen  $outputFile \n" . PHP_EOL, FILE_APPEND);}
...
if ($debug_log) {file_put_contents('debug.log', json_encode($response) . PHP_EOL, FILE_APPEND);}
