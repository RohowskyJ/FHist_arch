...
if ($debug_log) {
    file_put_contents('Findb_debug.log', "VF_Z_Suchb_findb.API L 063 DB verbunden " . PHP_EOL, FILE_APPEND);
}
...
if ($debug_log) {
    file_put_contents('Findb_debug.log', "L 0100 vor ar analyse " . PHP_EOL, FILE_APPEND);
}

foreach ($ar_arr as $ar_table => $ar) {

    if ($debug_log) {
        file_put_contents('Findb_debug.log', "L 014 var_table $ar_table ar $ar  " . PHP_EOL, FILE_APPEND);
    }
    ...
    if ($debug_log) {
        file_put_contents('Findb_debug.log', "L 0111 ar_table $ar_table " . PHP_EOL, FILE_APPEND);
    }

    $return_in = SQL_QUERY($db, $sql_in);
    $num_recs = mysqli_num_rows($return_in);
    if ($debug_log) {
        file_put_contents('Findb_debug.log', "VF_Z_Suchb_findb.API L 0115 ANz Sätzr $num_recs " . PHP_EOL, FILE_APPEND);
    }
    $find_sum = 0;
    $i = 0;
    while ($row = mysqli_fetch_object($return_in)) {
        ...
        if ($debug_log) {
            file_put_contents('Findb_debug.log', "L 063 anzahl der Sätze mit Eintragungen $i" . PHP_EOL, FILE_APPEND);
        }
        $i++;
    }
...
if ($debug_log) {
    file_put_contents('Findb_debug.log', "L 0159 vor dm analyse " . PHP_EOL, FILE_APPEND);
}
...
foreach ($dm_arr as $fo_table => $dm) {
    ...
    if ($debug_log) {
        file_put_contents('Findb_debug.log', "L 0165 fo_table $fo_table dm $dm " . PHP_EOL, FILE_APPEND);
    }

    $sql_in = "SELECT * FROM `$fo_table` ORDER BY md_id ";

    if ($debug_log) {
        file_put_contents('Findb_debug.log', "L 0165 Tabelle $fo_table wird eingelesen" . PHP_EOL, FILE_APPEND);
    }
    ...
    $num_recs = mysqli_num_rows($return_in);
    if ($debug_log) {
        file_put_contents('Findb_debug.log', "L 063 anzahl der Datensätze $num_recs " . PHP_EOL, FILE_APPEND);
    }
