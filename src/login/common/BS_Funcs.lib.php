...
if ($debug) {
    #echo " BS_Funcs.lib.php ist geladen. <br/>";
...
// --------------------------------------------------------------------------------
{
    ...
    $Store, $debug, $debug_log,  $path2ROOT, $ListHead, $TABU, $TABUcss;
    ...
    /*
    if (!isset($_SESSION['BS_Prim'])) {
        $_SESSION['BS_Prim'] = array('mode'=>'Mandanten','eignr'=>'',
            'p_uid'=>'','Inc_Arr'=> '','benutzer'=>'','zu_eignr'=>''
        );
    }
    */
    ...
    if (is_file($path2ROOT . 'login/common/config_s.ini')) {
        ...
        */
        if ($page_parm['Config']['cDeb'] != "") {
            $cDeb = $path2ROOT ."/login/logs/debug/".$c_Date."_".$page_parm['Config']['cDeb'];
        }
    }
    ...
    #echo " <link rel='stylesheet' href='" . $path2ROOT . "login/common/css/BS_Druck.css' type='text/css'>";  ist in add_N.css
    # echo " <link rel='stylesheet' href='" . $path2ROOT . "login/common/css/BS_TabSort.css' type='text/css'>"; 
    echo " <link rel='stylesheet' href='" . $path2ROOT . "login/common/css/opPopOver.css' type='text/css'>";
    #echo " <link rel='stylesheet' href='" . $path2ROOT . "login/common/css/pure.min.css' type='text/css'>";
    
    // Stayle für resp flex Header ( derzeit nur Print!)
    ?>
    <style>
    .page-header {
    max-width: 1200px;
...
@media (max-width: 600px) {
    ...
    <?php 
    if (strpos($_SERVER['PHP_SELF'],'_Edit') >= 1 ) {
    ...
    if ($type == 'Form') {
       ...
       # }
        ...
        // Beispiel: Einlesen der Konfigurationsdaten
        if (is_file($path2ROOT . 'login/common/config_s.ini')) {
        $ini_arr = parse_ini_file($path2ROOT . 'login/common/config_s.ini', true, INI_SCANNER_NORMAL);
        }
        $logo = isset($ini_arr['Config']['sign']) ? $ini_arr['Config']['sign'] : 'default-logo.png';
        $OrgName = isset($ini_arr['Config']['inst']) ? $ini_arr['Config']['inst'] : 'Organisation Name';
        ?>
