 ...
 */
$_SESSION[$module]['Inc_Arr'][] = "VF_S_Find_Auth_List.inc.php";
...
for ($i = 0; ! empty($arr_zeitg[$i]); $i ++) {
    ...
    echo "<pre class=debug style='background-color:lightblue;font-weight:bold;'>SU Arc List sql </pre>";
    echo "</div>";
    
    $return_fo = SQL_QUERY($db, $sql);
    while ($row = mysqli_fetch_object($return_fo)) {
        $zt_titel = $zt_index[$row->ih_zt_id];
        echo "<tr><td>$row->ih_id</td><td>$row->ih_titel</td><td>$row->ih_jahrgang-$row->ih_jahr-$row->ih_nr</td><td>$zt_titel</td><td>$row->ih_autor</td></tr>";
    }
}
echo "</table><br/>";
